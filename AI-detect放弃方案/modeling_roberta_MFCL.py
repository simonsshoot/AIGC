# modeling_roberta_gcl.py:# modeling_roberta.py
...
class RobertaForGraphBasedSequenceClassification(
    ...
    def __init__(self, config):
        ...
        self.graph_aggregation = GCNGraphAgg(
            config.hidden_size, config.task_specific_params["max_sentences"], config.task_specific_params["max_sen_replen"]
        )
    ...
    def forward(
        ...
        hidden_states = outputs.hidden_states  # outputs[2] in older versions

        graph_rep = self.graph_aggregation(
            hidden_states[-1],  # Assuming using the last layer's hidden states
            nodes_index_mask,
            adj_metric,
            node_mask,
            sen2node,
        ...
        outputs = (logits,) + outputs.hidden_states  # (logits, hidden_states...)
...
class RobertaForGraphBasedSequenceClassification_CL(BertPreTrainedModel):
    ...
    def forward(
        ...
        outputs = (logits,) + outputs.hidden_states
    ...
    def contrastive_loss_labelwise_winslide(
        ...
        for i in range(batch_size):
            ...
            numerator = torch.clamp(numerator, min=1e-6)
...
class EncoderForMBCL(BertPreTrainedModel):
    ...
    def forward(
        ...
        return outputs.hidden_states, whole_rep
...
class RobertaForGraphBasedSequenceClassification_RFCL(BertPreTrainedModel):
    ...
    def contrastive_loss_es(self, batch_size, batch_idx_by_label, hidden_feats):
        ...
        for i in range(batch_size):
            ...
            denominator = torch.sum(
                one_for_neg * torch.exp(sim_matrix[i, :] / self.temperature)
            )
...
class RobertaForContrastiveSequenceClassification(RobertaForGraphBasedSequenceClassification_CL):
    """
    基于 RobertaForGraphBasedSequenceClassification_CL 的对比学习模型。
    可以在此类中添加额外的层或功能以支持对比学习。
    """

    def __init__(self, config):
        super(RobertaForContrastiveSequenceClassification, self).__init__(config)
    ...
    def forward(
        ...
        outputs, whole_rep = super().forward(
            input_ids=input_ids,
            attention_mask=attention_mask,
            token_type_ids=token_type_ids,
            position_ids=position_ids,
