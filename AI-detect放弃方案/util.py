...
#         self.sen2node = sen2node
#         self.nodes_ent = nodes_ent
#         self.hyperedges = hyperedges  # 初始化超边
...
# #         # 提取 hyperedges
# #         # 使用 getattr 安全访问属性，避免 TypeError
# #         information = getattr(example, 'information', {})
# #         graph_info = information.get('graph', {}) if isinstance(information, dict) else {}
# #         hyperedges = graph_info.get('hyperedges', []) if isinstance(graph_info, dict) else []
...
#         self, with_relation, data_dir, train_file="gpt3.5_mixed_train_Adversarial_graph_exchange.jsonl"
#     ):
#         """See base class."""
...
#         self, with_relation, data_dir, dev_file="3.5mix_val_InsertAdv_pct10_graph.jsonl"
#     ):
#         """See base class."""
#         return self._create_examples(
...
#         self, with_relation, data_dir, test_file="3.5mix_val_InsertAdv_pct10_graph.jsonl"
#     ):
#         """See base class."""
#         return self._create_examples(
...
class InputExample(object):
    ...
    Args:
        ...
        label: (Optional) string. The label of the example. This should be
        specified for train and dev examples, but not for test examples.
        nodes_index: (Optional) list. Node indices.
        adj_metric: (Optional) list. Adjacency metrics.
        all_tokens: (Optional) list. All tokens in the example.
        sen2node: (Optional) list. Sentence to node mapping.
        nodes_ent: (Optional) list. Node entity information.
        hyperedges: (Optional) list. Hyperedges in the graph.
    """
    ...
    def __init__(
        ...
        self.sen2node = sen2node
        self.nodes_ent = nodes_ent
        # self.hyperedges = hyperedges  # 存储超边信息
...
def glue_convert_examples_to_features(
    ...
    for ex_index, example in enumerate(examples):
        ...
        # hyperedges = getattr(example, 'hyperedges', [])
...
class DeepFakeProcessor(DataProcessor):
    """Processor for the DeepFake data set."""

    def get_example_from_tensor_dict(self, tensor_dict):
        """See base class."""
        return InputExample(
            guid=tensor_dict["idx"].numpy(),
            text_a=tensor_dict["sentence1"].numpy().decode("utf-8"),
            text_b=tensor_dict["sentence2"].numpy().decode("utf-8"),
    ...
    def get_dev_examples(
        self, with_relation, data_dir, dev_file="gpt3.5_mixed_val_Adversarial_graph.jsonl"
    ):
        """See base class."""
        return self._create_examples(
            with_relation, self._read_jsonl(os.path.join(data_dir, dev_file)), "val"
    ...
    def get_test_examples(
        self, with_relation, data_dir, test_file="gpt3.5_mixed_val_Adversarial_graph.jsonl"
    ):
        """See base class."""
        return self._create_examples(
            with_relation, self._read_jsonl(os.path.join(data_dir, test_file)), "test"
    ...
    def _get_nodes(self, nodes):
        """Extracts node indices and entity information."""
        all_nodes_index = []
        all_nodes_ent = []
        for node in nodes:
            all_nodes_index.append(node["spans"])
    ...
    def _get_adj_metric(self, edges, drop_nodes, node_num, with_relation):
        """Generates adjacency metrics based on edges and relations."""
        if with_relation == 0:
            adj_matrix = np.eye(node_num)
            for edge in edges:
                adj_matrix[edge[0], edge[1]] = 1
            ...
            adj_matrix = np.stack(adj_matrix, axis=0)
            for edge in edges:
                r_i = relation.get(edge[2], 0)  # 默认关系为0
                adj_matrix[r_i][edge[0], edge[1]] = 1
                adj_matrix[r_i][edge[1], edge[0]] = 1
    ...
    def _create_examples(self, with_relation, inputs, set_type):
        ...
        for i, line in enumerate(lines):
            line = line.strip()
            if not line:
                logger.warning(f"Skipping empty line at {i + 1}")
                continue
            try:
                line = json.loads(line)
            except json.JSONDecodeError as e:
                logger.error(f"JSONDecodeError at line {i + 1}: {e}")
                logger.error(f"Invalid line content: {line}")
                continue

            # 检查 'split' 字段是否匹配
            if "split" not in line.keys() or line["split"] == set_type:
                guid = f"{set_type}-{i}"
                text_a = line.get("article", "")
                text_b = None
                label = line.get("label", "machine")  # 默认标签为 "machine"

                # 提取图信息
                information = line.get("information", {})
                graph_info = information.get("graph", {})
                nodes = graph_info.get("nodes", [])
                edges = graph_info.get("edges", [])
                all_tokens = graph_info.get("all_tokens", [])
                drop_nodes = graph_info.get("drop_nodes", [])
                sen2node = graph_info.get("sentence_to_node_id", [])
                # hyperedges = graph_info.get("hyperedges", [])  # 提取超边信息
                ...
                if len(all_tokens) != 0:
                    ...
                    bad += 1
                    logger.warning(f"Skipping example {guid} due to empty 'all_tokens'")
                    continue

        logger.info(f"\n {bad} instances have no 'all_tokens' and were skipped.")
        return examples
...
def glue_compute_metrics(task_name, preds, labels):
    ...
    if task_name == "cola":
        ...
        return {"acc": simple_accuracy(preds, labels)}
    elif task_name in ["mrpc", "deepfake"]:
        return acc_and_f1(preds, labels)
    elif task_name == "sts-b":
        return pearson_and_spearman(preds, labels)
    elif task_name == "qqp":
